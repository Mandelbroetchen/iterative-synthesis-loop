...
**Objective**: Generate or update code files in `code-raw/` according to explicit instructions in `prompt-final/.overwrite.json`, using design artifacts as the single source of truth.
...
### 1.5 Code Generation Instructions
**File**: `prompt-final/.overwrite.json`

This JSON file explicitly specifies which code files to generate and how:

```json
{
  "files": [
    {
      "path": "code-raw/module.py",
...
```
<insert src="./prompt-final/.overwrite.json"></insert>
```
...
These files currently exist in `code-raw/` and represent the current implementation state. Update them according to the modes specified in `.overwrite.json`.
...
### Rules
1. **Only generate/update files listed in `.overwrite.json`**
...
- All files listed in `.overwrite.json` must be included in the `files` dictionary
